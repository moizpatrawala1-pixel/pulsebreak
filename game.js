(() => {
  ...
  // Classify device tier at startup so we can scale rendering costs accordingly.
  const PERF = (() => {
    ...
    return {
      shadowBlur: !isLowEnd,         // shadowBlur is GPU-expensive — skip on low-end
      maxParticles: isLowEnd ? 80 : isMidRange ? 130 : 180,
      maxSparks: isLowEnd ? 30 : isMidRange ? 55 : 80,
      burstScale: isLowEnd ? 0.55 : isMidRange ? 0.78 : 1.0,
      dprCap: isLowEnd ? 1 : 2,      // cap pixel ratio on low-end — halves fill cost
    };
  })();
  ...
  function xpForLevel(level) { return Math.round(80 + Math.pow(level, 1.42) * 42); }
  function xpAtLevel(level) { let t = 0; for (let i = 1; i < level; i++) t += xpForLevel(i); return t; }
  function levelFromXp(xp) {
    let level = 1, remaining = xp;
    while (level < 100 && remaining >= xpForLevel(level)) { remaining -= xpForLevel(level); level++; }
    return level;
  }
  ...
  function currentStage() {
    ...
    return { ...base, number: state.stageIndex, speedMul: base.speedMul + loops * 0.38, duration: base.duration + loops * 5, pressure: loops ? `${base.pressure} +${loops}` : base.pressure };
  }
  ...
  function renderMissions() {
    ...
    save.missions.forEach((mission) => {
      ...
      item.innerHTML = `<span class="mission-copy">${mission.label} <span class="muted">${format(progress)}/${format(mission.target)}</span></span><span class="mission-reward">${mission.paid ? "Paid" : `+${mission.reward}`}</span>`;
  ...
  function skillCost(node, nextRank) {
    return { coins: Math.round(node.baseCoins * Math.pow(nextRank, 1.45)), cores: Math.round(node.baseCores * Math.pow(nextRank, 1.35)) };
  }
  ...
  function buySkill(node) {
    ...
    save.coins -= cost.coins; save.cores -= cost.cores;
  ...
  function handleCosmetic(item, bucket, selectedKey) {
    if (save.unlocked[bucket][item.id]) {
      save[selectedKey] = item.id; playSound("select"); toast(`${item.name} equipped`); persist(); return;
    }
  ...
  function unlockCosmetic(bucket, id) {
    ...
    save.unlocked[bucket][id] = true; normalizeSave(save);
  }
  ...
  function claimDailyReward() {
    ...
    toast(`Claimed ${reward}`); playSound("buy"); persist();
  }
  ...
  function startRun(mode = "classic") {
    ...
    if (mode === "daily") {
      ...
      state.seed = save.challenge.seed; state.ghostReplay = null;
    } else {
      state.seed = Math.floor(Math.random() * 2 ** 32); state.ghostReplay = null;
    }
  ...
  function progressMissions(missions) {
    ...
    missions.forEach((m) => {
      if (m.paid) return;
      const amount = m.metric === "score" || m.metric === "maxCombo" || m.metric === "stageReached"
  ...
  function progressWeeklies() {
    save.weekly.forEach((c) => {
      ...
      if (!c.done && c.progress >= c.target) {
        c.done = true; c.paid = true; addXp(c.xp); save.cores += c.cores; save.stats.weeklyChallenges += 1;
      }
    });
  }
  ...
  function updateAchievements() {
    ...
    ACHIEVEMENTS.forEach((ach) => {
      ...
      if (achievementProgress(ach) >= ach.target) {
        save.achievements[ach.id] = true; grantAchievementReward(ach); unlocked++;
      }
    });
  ...
  function saveGhostIfBest() {
    ...
    const prev = save.ghosts[zoneId];
    if (!prev || state.run.score > prev.score) {
      save.ghosts[zoneId] = { score: state.run.score, stage: state.run.stageReached, seed: state.seed, lanes: state.inputGhost.slice(0, 900), created: Date.now() };
  ...
  async function luckySpin() {
    ...
    if (!rewarded) return;
    countAd("spin");
    const rank = bonus("lucky"); const roll = Math.random();
    if (roll < 0.36 - rank * 0.03) { const amount = 400 + rank * 100; save.coins += amount; toast(`Lucky Spin: ${amount} coins`); }
  ...
  function triggerBoost() {
    ...
    state.energy = 0; state.overdrive = 3.6 + bonus("boostDuration") * 0.12;
    state.combo = Math.max(state.combo, 3); state.run.boosts += 1;
  ...
  function update(dt) {
    if (state.screen !== "playing") { updateParticles(dt); return; }
    ...
    updateObstacles(dt); updateShards(dt); updateParticles(dt);
    maybeAddSpeedSparks(dt, stage); updateHud();
  }
  ...
  function spawnStagger() {
    ...
    for (let i = 0; i < 3; i++) state.obstacles.push({ lanes: [clamp(lane + i * dir, 0, state.lanes - 1)], y: -56 - i * 72, h: 30, kind: "stagger", passed: false, hit: false, elite: false });
  ...
  function spawnChaos() { spawnDouble(); if (state.rng() > 0.5) spawnElite(); }
  ...
  function updateParticles(dt) {
    ...
    for (const s of state.sparks) { s.y += s.vy * dt; s.life -= dt; }
  ...
  function drawBackground(now) {
    ...
    for (let y = -spacing; y < state.h + spacing; y += spacing) {
      ctx.beginPath(); ctx.moveTo(0, y + offset); ctx.lineTo(state.w, y + offset + 28); ctx.stroke();
    }
    ctx.restore();
    ctx.save();
    ctx.globalAlpha = 0.14 + Math.sin(now * 1.5) * 0.04;
    ctx.fillStyle = zone.color;
    ctx.beginPath(); ctx.arc(state.trackX + state.trackW * 0.12, state.h * 0.18, 120, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 0.12;
    ctx.fillStyle = "#ffd166";
    ctx.beginPath(); ctx.arc(state.trackX + state.trackW * 0.82, state.h * 0.86, 150, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }
  ...
  function drawTrack(now) {
    ...
    ctx.strokeStyle = selectedSkin().color; ctx.lineWidth = state.overdrive > 0 ? 3 : 1.5;
    ...
    ctx.fillStyle = scan; ctx.fillRect(x, scanY - 80, w, 160);
    ctx.restore();
  }
  ...
  function drawGhost() {
    ...
    // No shadowBlur on ghost — flat fill only
    ctx.beginPath(); ctx.arc(x, state.playerY + 18, 16, 0, Math.PI * 2); ctx.fill();
    ctx.restore();
  }

  function drawObstacles(now) {
    // ── Batch obstacles by kind to minimise state changes ──
    // Group into elite and regular to do one shadow pass each.
    const regular = [], elite = [];
    for (const ob of state.obstacles) {
      if (ob.elite) elite.push(ob); else regular.push(ob);
    }

    if (PERF.shadowBlur) {
      drawObstacleGroup(regular, now, false);
      drawObstacleGroup(elite, now, true);
    } else {
      // Low-end path: flat rects with a bright highlight strip — no shadowBlur
      drawObstaclesFlat(state.obstacles, now);
    }
  }

  function drawObstacleGroup(obs, now, isElite) {
    if (!obs.length) return;
    ctx.save();
    const groupColor = isElite ? "#a476ff" : null;
    for (const ob of obs) {
      const color = groupColor || (ob.kind === "wall" ? "#ff5a72" : ob.kind === "boss" ? "#ff334d" : "#ff8a55");
      for (const lane of ob.lanes) {
        ...
        ctx.shadowColor = color; ctx.shadowBlur = 18 * glow;
        const grad = ctx.createLinearGradient(x, y, x + w, y + ob.h);
        grad.addColorStop(0, color); grad.addColorStop(1, "#23070c");
        ctx.fillStyle = grad;
        roundRect(ctx, x, y, w, ob.h, 7); ctx.fill();
        ctx.shadowBlur = 0;
  ...
  function drawObstaclesFlat(obs, now) {
    // No shadowBlur at all — replace glow with a bright top edge strip
    ctx.save();
    for (const ob of obs) {
      ...
      for (const lane of ob.lanes) {
        ...
        ctx.fillStyle = grad;
        roundRect(ctx, x, y, w, ob.h, 7); ctx.fill();
        // Bright strip simulates glow cheaply
  ...
  function drawShards() {
    ...
    for (const shard of state.shards) {
      ...
      if (PERF.shadowBlur) { ctx.shadowColor = shard.rare ? "#ffffff" : "#ffd166"; ctx.shadowBlur = 22; }
      ctx.fillStyle = shard.rare ? "#ffffff" : "#ffd166";
      roundRect(ctx, -r, -r, r * 2, r * 2, 3); ctx.fill();
      ctx.shadowBlur = 0;
      ctx.globalAlpha = 0.4; ctx.fillStyle = shard.rare ? "#37f4ff" : "#ff5a72";
  ...
  function drawPlayer(now) {
    ...
    ctx.save();
    ctx.translate(x, y);
    if (PERF.shadowBlur) { ctx.shadowColor = state.overdrive > 0 ? skin.accent : skin.color; ctx.shadowBlur = state.overdrive > 0 ? 42 : 24; }
    const trail = ctx.createLinearGradient(0, 18, 0, 88);
    trail.addColorStop(0, `${skin.color}aa`); trail.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = trail;
    ctx.beginPath(); ctx.moveTo(-12, 12); ctx.lineTo(0, 92 + Math.sin(now * 12) * 12); ctx.lineTo(12, 12); ctx.closePath(); ctx.fill();
    ctx.fillStyle = skin.color;
    ctx.beginPath(); ctx.moveTo(0, -24); ctx.lineTo(22, 10); ctx.lineTo(0, 24); ctx.lineTo(-22, 10); ctx.closePath(); ctx.fill();
    ctx.shadowBlur = 0;
    ctx.fillStyle = "#ffffff";
    ctx.beginPath(); ctx.moveTo(0, -12); ctx.lineTo(8, 7); ctx.lineTo(0, 14); ctx.lineTo(-8, 7); ctx.closePath(); ctx.fill();
    ctx.fillStyle = skin.accent; ctx.fillRect(-4, 4, 8, 12);
    ctx.restore();
    ctx.globalAlpha = 1;
  }
  ...
  function drawParticles() {
    ...
    for (const p of state.particles) {
      ...
      // Skip shadowBlur on particles — they're already small and numerous
  ...
  function drawMessages() {
    ...
    for (const m of state.messages) {
      ...
      if (PERF.shadowBlur) { ctx.shadowColor = m.color; ctx.shadowBlur = 14; }
  ...
  // ── Input handling ────────────────────────────────────────────────────────────
  // Uses a hybrid model:
  //   • Tap/hold: direct lane-from-X (original behaviour)
  //   • Swipe: relative swipe detection — drag right = move right one lane, etc.
  //     This feels more natural on phone and avoids "fat-thumb" lane misreads
  //     on small screens where the tap target per lane is only ~60-70px wide.

  const SWIPE_THRESHOLD = 22; // px — minimum horizontal drag to count as swipe
  ...
  function handlePointerDown(event) {
    ...
    // Record swipe start so we can detect relative direction
    state.swipeStartX    = event.clientX;
    state.swipeStartLane = state.targetLane;
    // Also set lane directly (existing tap behaviour preserved)
  ...
  // passive: true on all canvas touch/pointer events — prevents scroll-jank warnings
  // and gives the browser permission to handle scrolling without waiting for JS
